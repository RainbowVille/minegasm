...
//            LOGGER.info(state.getKey() + ": " + state.getValue().getIntensity());
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (event.phase == TickEvent.Phase.END && isPlayer(event.player)) {
            ...
            if (tickCounter % MinegasmConfig.tickFrequency.getInt() == 0) // TODO: Add ticks per second config option (Default: Every tick)
            {
                tickAll();
    ...
    {
        if (isPlayer(event.getEntityPlayer()))
        {
    ...
    @SubscribeEvent
    public static void onXpChange(PlayerPickupXpEvent event)
    {
        if (isPlayer(event.getEntityLiving()))
        {
            ((VibrationStateXpChange)vibrationStates.get("xpChange")).onXpChange(((EntityPlayer)event.getEntityLiving()).experienceTotal, event.getOrb().xpValue);
        }
    }
    ...
    public static void onRespawn(PlayerRespawnEvent event)
    {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            try {
                ...
                if (uuid.equals(Minecraft.getMinecraft().player.getGameProfile().getId())) {
